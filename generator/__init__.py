...
import generator.IdGenerator as idgen
import generator.StatusGenerator as statgen
import generator.CurrencyGenerator as curgen
import generator.DirectionGenerator as dirgen
import generator.DescriptionGenerator as descgen
import generator.TagGenerator as taggen
import generator.TimestampGenerator as dategen
import generator.PriceGenerator as pricegen
import generator.VolumeGenerator as volgen
import config
import util
from converter import OrderRecordToMySQLInsert
...
def setup_generators():
    """
     ...
     All data related to ids etc. can be found in generator.constants
    """
    ...
    try:
        ...
        util.logger.log_error(traceback.format_exc())
    else:
        util.logger.log_info("Generators initialized successfully")
...
def create_order(position):
    """
    Creates order with data generator
    """
    order = dict()
    try:
...
def get_orders_sequence():
    ...
    """
    util.logger.log_info("Generating {} orders...".format(orders_amount))
    i = 0
    try:
        for i in range(orders_amount):
            order = create_order(i)
            util.logger.log_trace(order)
            yield order
    except:
        util.logger.log_error(traceback.format_exc())
    finally:
        util.logger.log_info("Generated {} orders...".format(i + 1))
        ...
        if __is_weekend(timestamp):
            if status != consts.statuses[0]:
                ...
                while __is_weekend(timestamp):
                    timestamp += 86400
                util.logger.log_trace("[Order][{}] Add {} sec to timestamp: {}. Previous value {} was a weekend".format
...
def get_notes_sequence(orders_sequence=get_orders_sequence()):
    ...
    """
    setup_generators()
    util.logger.log_info("Generating order notes...")
    i = 0
    try:
        for order in orders_sequence:
            for item in generate_notes_from(order):
                util.logger.log_trace(item)
                i += 1
                yield OrderRecordToMySQLInsert.convert(item)
    except:
        util.logger.log_error(traceback.format_exc())
    finally:
        util.logger.log_info("Generated {} order notes...".format(i))
